...
<script>
  ...
  export default {
    ...
    async asyncData () {
      let { data } = await axios.get('/api/insurance/product/list/query.json?access_token=XqTest9c5a71a1802952b59c5d137a8aecaadf1045feaa')
      return {
    ...
    methods: {
      fetchData () {
        // fetch('https://api.xueqiu.com/insurance/product/list/query.json?access_token=7a6dd1ad235f38fa23379dab2d219cc29457337c')
        fetch('/api/insurance/product/list/query.json', {credentials: 'same-origin'})
          .then(response => response.json())
          .then(json => {
            // console.log('product list', json.data.items)
            if (json.error_code) {
              alert(`${json.error_description} ${json.error_code}`)
            }
            this.productList = json.data.items
          })
      },
      clickTab (tab) {
